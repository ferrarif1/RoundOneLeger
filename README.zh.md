...
## 概述
RoundOneLeger 是一套开源免费的内网协同办公系统，支持共享表格与共享文档在线编辑，用于集中管理 IP 地址、人员、系统以及配套的审计数据，并引入协同工作台功能。后端使用 Go 编写并集成了 Gin 兼容路由，前端则采用 React 与 Tailwind 风格的样式，提供贴近 Eidos 设计语言的管理控制台。
